  ...
  <section className="py-20 px-[5%] bg-background">
    ...
    >
      <h2 className="font-heading text-[clamp(1.9rem,4vw,3rem)] font-black leading-tight text-foreground">
        Graded Music Exams With <span className="text-primary">Top Universities</span>
      </h2>
    </motion.div>
    ...
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-5 max-w-275 mx-auto">
      {exams.map((exam, i) => (
        ...
        >
          <div>
            <h3 className="text-white font-bold text-[1rem] leading-tight">{exam.name}</h3>
            {exam.location && <p className="text-white/70 text-[.85rem] mt-0.5">{exam.location}</p>}
          </div>
          {exam.isLogo && exam.logo ? (
            <Image src={exam.logo} alt={exam.name} className="w-14 h-14 rounded-full object-cover border-2 border-white/20 shrink-0" />
          ) : (
